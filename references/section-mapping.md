...
> 本表是脚本化抽取章节内容时的关键词字典。**所有抽取脚本必须用 fuzzy match,不能依赖固定数字编号**。否则会漏掉 1/3 以上的文献(实际案例:V3 摘抄因只匹配 `## 3`/`## 10` 阿拉伯数字编号,导致 8 篇文献的章节为空)。

---

## 一、章节编号风格(同一份文件内通常一致,但 18 篇文献之间会混用)
...
`vlms-identify-distracted-driver-naturalistic-videos.md` 用的是 `## 背景与动机 (Background & Motivation)`(没有"研究"前缀)。如果只匹配关键词 `研究背景`,这篇会出空白。
**修复**:增加 `背景与动机` 作为 fallback。

### 案例 2:`## 5. 方法论:HSUM 框架详解` 漏抽

如果只匹配 `^## 5. 方法论$`(精确匹配),这种带子标题的会漏。
**修复**:用 `index($0, "方法")` 包含式匹配。

### 案例 3:`## 三、研究背景(Background)` 漏抽

如果只匹配 `## 3. 研究背景`(数字 + 点),中文数字编号的版本会全部漏掉。
